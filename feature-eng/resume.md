...
2. 读该主题 `progress.yaml`：path、run_mode、invoke、stage、gates、artifacts。缺 `run_mode`/`invoke` 时按 `guided` / `defaults.invoke|strict` 对待并建议补写。
3. 若存在 `handoff.md` → 先读它恢复上下文。
4. 向用户播报：当前环、run_mode/invoke、已完成产物、未过闸、下一步动作（字段同 [status.md](status.md) 第 3 点）。
5. 按当前 stage 继续：
   - 当前环需子 skill → 按 [binding.md](binding.md) lookup 调起（尊重 `progress.invoke`）
   - 当前环是闸 → 按 [gates-common.md](gates-common.md) 校验
   - 用户声称本环完成 → [advance.md](advance.md)（口令协议见该页）
