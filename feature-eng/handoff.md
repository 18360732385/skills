...
- 单主题跨多次会话推进
- **主动提议**（控制器应开口，不必等用户点名）：
  - 本主题已完成环数 ≥ 5（含已跳过环的「已处理」计数亦可），或
  - 单会话明显接近上下文上限 / 回复开始丢环节约束，或
  - advance 刚推进且下一环仍长（如即将 implement）

主动提议话术示例：「建议先写 `handoff.md` 并新会话 `feature-eng resume`，避免后续环丢闸。」用户拒可继续。
