...
人读入口：[README.md](README.md)。验收：[VERIFY.md](VERIFY.md)。烟测：`node scripts/selfcheck.mjs`。

**拓扑（0.2.4-dev）**：根目录模式 md（init/rebind/start/resume/status/advance/close）· `config/` 绑定 SSOT · `templates/` 过程态骨架。控制器本身不做具体开发。
...
| 绑定 lookup / invoke / 指针卡片 | [binding.md](binding.md) · [config/stage-bindings.yaml](config/stage-bindings.yaml) |
...
| 新主题开工 / 续跑 / 状态 | [start.md](start.md) · [resume.md](resume.md) · [status.md](status.md) |
