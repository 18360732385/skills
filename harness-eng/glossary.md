...
大仓【推荐】`all_domains` + `strict`；贴顶后升 `gold`（[recommended-profile.md](modes/recommended-profile.md)）。详 [fill-score.md](fill/fill-score.md) / [fill-gate.md](fill/fill-gate.md)。
...
| 说法 | 含义 |
|---|---|
| **契约域** | 注册表 `domains.yaml` 中 `kind: contract` 的域（现含 func/api/db/redis/jobs…）。名单 / 路径 / CLI 默认由 registry 驱动（0.3.7+）；morph 必填章见 `morph-required.yaml`（0.3.8+）；L1 文件靠 **packs** 展开。见 [domain-extend.md](modes/domain-extend.md) |
...
| **报告字段** | `gap_to_ready` / `domain_stories` / `diff` / `suggest_upgrade` 等 → [fill-score.md](fill/fill-score.md)「报告字段速查」 |
| **安全预填** | `Q_SEED`：用无密可推断信息填 Commands/技术栈；不确定仍 TODO |
| **确认闸门** | 写盘前必须用户明确确认；词表 SSOT：[write-plan.md](modes/write-plan.md) |
| **预授权** | 首轮确认后，同会话后续 fill/resume 可自动写盘；词表与细则：[write-plan.md](modes/write-plan.md) |
