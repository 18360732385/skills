...
**当前版本：0.6.4-dev**（见 [CHANGELOG.md](CHANGELOG.md)、[QUICKSTART.md](QUICKSTART.md)；权威号：`templates/_meta/manifest.yaml`）

**0.6.4-dev**：CodeBuddy/WorkBuddy 官方对齐（扁平 `.codebuddy/rules/*.md` 保留 FM；hooks `/hooks` 面板；MCP/permissions 文档）。**0.6.3**（正式）：消费仓 `sync.mjs` freshness gate；报告壳认 `report_schema`；生产装/升 **`main`**。**0.6.2**：会话仪表盘去掉 mermaid（Trae Syntax Error），改纯文本态势。**0.6.1**：Trae **高**（[host/TRAE-P0-EVIDENCE.md](host/TRAE-P0-EVIDENCE.md)）。**0.6.0 计划**（已收口）：[ROADMAP-0.6.0.md](ROADMAP-0.6.0.md)  
...
见 `templates/_meta/manifest.yaml` 的 `version` 字段（当前 **0.6.4-dev**）。报告对照 **`skill_version` + `report_schema`**（**报告壳 ≠ skill**；`ui.version` 仅为兼容别名，见 glossary）。
