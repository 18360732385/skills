...
harness-eng **0.6.1**（0.6.0 列车已收口；0.6.1-dev spike 已钉号）对齐矩阵：Trae **高**，Cursor（及 Claude / Qoder / WorkBuddy）**高**。  
...
| MCP | `.cursor/mcp.json` | `.trae/mcp.json`（项目 MCP / Settings 开关以官方为准） | 启用条件写死、fill 路径对齐 |
| Skills | `.cursor/skills/` 一等公民 | `.trae/skills/` **一等公民**（docs PASS；适配卡已去掉「若宿主支持」） | 真人会话确认可发现 / 可点名 |

「高」**不是**「改成 Cursor 扁平 `hooks.json`」。禁止事项仍见适配卡：**勿用** `beforeShellExecution` 顶层扁平格式。**不假装** Trae 走 Cursor 协议。
...
- L4+ `.trae/mcp.json.example`；真密 `.trae/mcp.json`；`mcp-paths.mjs` / fill-mcp / calibrate-live 已列该路径 **+ IDE Settings 启用**
- L3 总是 `.githooks/pre-commit` + gate `--git`
- `scripts/fixtures/multi-host-hooks` 含 Trae hooks 信号；L5 `sync.mjs` **能**分发 `.trae/`（见 fixture 内同步本）

P0 spike（0.6.1-dev → 0.6.1）已落地的工程项：
...
- 项目 MCP：**IDE 已消费** `.trae/mcp.json`（T-P0-2 **PASS**，2026-09-14 面板：12 台在场；3 开、其余 toggled off）。早先「缺 7 台」是误读
- hooks：T-P0-3 / T-P1-2 **2026-09-14 live PASS**（c-be-sms-ai：Settings → Hooks 已开；matcher `Bash|RunCommand`；`additionalContext` 注入；软 allow）。Round C **本机行为 FAIL** 归档为 **matcher 误诊**。配方仍写 **新会话**（权威仓当次已加载 `hooks.json`，非硬性）。`.githooks` 仍兜底。hooks PASS **单独不授权**升 **高**
- 矩阵 中高→高（T-P0-2 / T-P1-3 / T-P1-4 / T-P1-5 已勾）
...
| **T-P0-2** project MCP / Beta | `.trae/mcp.json` 是否必须打开项目 MCP / Settings 开关；与 Cursor `.cursor/mcp.json` 的操作差在哪 | 官方或实测：启用开关、路径、未开时的失败形态 | **PASS**（2026-09-14 MCP 面板）：**IDE 已消费**文件。Settings 显示 **12** 台。**ON**：gitlab / chrome-devtools / Apifox 导入。**OFF via toggle**（在场）：sonarqube、redis-*、mysql-*。启用 = Settings 开关，不是缺文件 |
...
| **T-P1-2** hooks + githooks | 原生 Claude 族 hooks 与 L3 `.githooks` 兜底一起对 Trae 可靠 | 不改族、只修 matcher：`TRAE_STYLE` 门禁 **`RunCommand`**（`Bash\|RunCommand` 兼容）；adapter 软放行补 `additionalContext`；gate 类仍有 `--git` 兜底。**生成器已落地** + **2026-09-14 真人 live PASS**。hooks PASS **单独不授权**升 **高** |
| **T-P1-3** MCP docs + fill path | 文档与 fill 路径对齐 T-P0-2 | [ai-tools.md](ai-tools.md) · [fill-mcp.md](../fill/fill-mcp.md) · `mcp-paths.mjs` · calibrate-live 优先级写清 `.trae/mcp.json` 与 Settings 启用；gitignore snippet 已有则不重复发明 |
| **T-P1-4** fixtures | 补 Trae 黄金集 | 新增 `mature-trae`（对照 `mature-claude`：根 AGENTS + `.trae/rules` + 契约骨架 + agent-kb，**无** `.cursor/rules` 仍判 MATURE）；selfcheck 钉 L5 sync 路径（`.trae/rules` / hooks / mcp + `l5-trae-ssot-00-harness`）；不另造巨型黄金树 |
| **T-P1-5** matrix bump | 矩阵 Trae **中高 → 高** | 仅当 P0 全过且 T-P1-1…4 绿；同步改 [ai-tools.md](ai-tools.md) · [adapters/trae.md](../templates/ai-tools/adapters/trae.md) · [sync-hosts.md](sync-hosts.md) · QUICKSTART / 手册矩阵句；**同时**改 selfcheck 里「marks Trae as 中高」类断言 |

## P2 体验抛光

不挡矩阵升级；可与 P1 并行收尾。Trae 已是 **高** 后，脚注强度可弱于 Codex。
...
**版本**：**0.6.1**（0.6.1-dev Trae P0 spike 已钉号）。  
**不要**重开已收口的 **0.6.0**（M1–M4 已完成）。

## 「同级」验收清单

全部勾上才允许 T-P1-5 把矩阵写成 **高**。本版已齐。

- [x] T-P0-1：官方加载路径 + FM 已记录；harness 停剥 FM；**消费仓刷新后**磁盘+行为 PASS（Round A）；升级后仍须刷新
- [x] T-P0-2：项目 MCP 路径已写；**IDE 已消费**文件；**MCP 面板 PASS**（2026-09-14：12 台在场，3 开、其余 toggled off）
- [x] T-P0-3：hooks **docs + 本机行为 PASS**（2026-09-14 c-be-sms-ai：Settings → Hooks；matcher `Bash|RunCommand`；`additionalContext`；软 allow）。Round C **本机行为 FAIL** 归档为 **matcher 误诊**
...
- [x] T-P1-2：原生 hooks + `.githooks` 对 Trae 可回归（生成器 + 2026-09-14 live PASS）。hooks PASS **单独不授权**升 **高**
- [x] T-P1-3：MCP 文档与 `mcp-paths` / fill-mcp / calibrate-live 一致（`.trae/mcp.json` + IDE Settings 启用）
- [x] T-P1-4：`mature-trae` + Trae L5 sync 路径 selfcheck 钉（薄断言，无巨型黄金树）
- [x] T-P1-5：矩阵 / 适配卡 / QUICKSTART / 手册 / selfcheck「中高」断言同步改为 **高**
- [x] 仍未假装 Trae 使用 Cursor 协议（适配卡「禁止」条仍在）
- [x] 0.6.0 列车未被重开；升号 **0.6.1**（0.6.1-dev spike 已钉号）
