...
2026-09-12 Trae CN 回传见实证页「实机回传」。权威仓 **c-be-sms-ai**：**Round A** 刷新后 T-P0-1 **磁盘 + 行为 PASS**（刷新前磁盘 FAIL，根因仍是实例化 `sync.mjs` 仍旧）。**Round C** T-P0-3 **本机行为 FAIL**。先做第 0 节再验规则。
...
**已知 SSOT 缺口（冻结 `HOOK_DEFS`）**：本机工具名是 `Shell` / `run_mcp` / `Edit` / `Write`；matcher 写 `Bash` / `mcp__mysql` / `MultiEdit`。即便宿主日后调用 hooks，`Bash` 也对不上 `Shell`。政策不变：只在「`Bash` 无 / `RunCommand` 有」时改 matcher。Round C **两者皆无**（宿主未调用）→ 改名是 T-P1 候补，等宿主先打中任意一条。

**探测配方**（2026-09-12 实机用过；保留）：

1. **新开** Trae Agent 会话（中途改 `hooks.json` **可能不热加载**；上半场因此 inconclusive。Round C 是 revert 后同会话，差分仍强，但协议偏好全新会话）
2. stage 一份会被门禁盯到的生成物（例：`.claude` 下 GENERATED 文件）
3. 让 Agent 跑 `git commit --dry-run`（走 **Trae** 终端 / 软门禁，不是 Cursor Shell）
...
2026-09-12 Round C（正确通道 `.trae/hooks.json`）：手工 stdin Claude payload ✓ 注入；live `Shell` + `git commit --dry-run` ✗；live Exec `RunCommand` ✗。**T-P0-3 本机行为 FAIL**。上半场 `Bash` / 中途临时 `RunCommand` 亦未见 systemMessage。

只要「`Bash` 不触发、`RunCommand` 触发」成立，T-P1-2 才改 harness matcher。宿主一条都没打中之前，**不**改 `HOOK_DEFS`。
...
T-P0-3 hooks：通道=.trae/hooks.json（勿报 .cursor） / 是否新会话=是|否 / Bash=触发|否 / RunCommand=触发|否 / live Shell=… / Stop=…
