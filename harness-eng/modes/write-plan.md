...
### Codex 期望（`ai_tools` 含 `codex` 时强制展示）
...
确认后优先跑 `scripts/harness.mjs`（`--mode land|resume|upgrade|pipeline-skeleton`）。非 L5 委托 `render.mjs`；`agent_config: true` 只渲 SSOT，**拒绝**直渲 `.cursor/rules` 等生成宿主路径，改走目标仓 `node scripts/agent-config/sync.mjs`。字段与展开语义以 `harness.mjs --help` 为准（`render.mjs --help` 仅内部渲染器）。要点：
