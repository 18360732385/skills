...
- [ ] 全量镜像宿主（claude/qoder/trae/workbuddy）与 **L3+/L5 Codex** **omit** 冗余 `1x-contract-sync` / `contract-sync`
- [ ] 若选用 Codex：已告知 **高（推荐纪律 B）** — `sync.mjs` 发出 Starlark rules / hooks / MCP toml / `.agents/skills`；**不做** `.mdc` 镜像（见 [adapters/codex.md](../templates/ai-tools/adapters/codex.md) · [CODEX-PARITY.md](../host/CODEX-PARITY.md)）
