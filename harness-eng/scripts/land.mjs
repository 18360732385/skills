#!/usr/bin/env node
/**
 * Unified land / resume / upgrade write entry (0.5.9+).
 *
 * Usage:
 *   node scripts/land.mjs --root <TARGET> --params <params.json>
 *       [--mode land|resume|upgrade] [--dry-run] [--manifest <path>] [--backup]
 *       [--no-sync]
 *
...
Canonical Agent write entry (0.5.9+).
...
function main() {
  ...
  if (!["land", "resume", "upgrade"].includes(mode)) {
    throw new Error(`Unknown --mode ${args.mode} (use land|resume|upgrade)`);
  }
  ...
  console.error(
    `land: agent_config=true（mode=${mode}）。生成宿主路径由 sync 发出，render 不得直写 .cursor/rules 等。`
  );
  ...
  if (args.dryRun) {
    console.error("land: --dry-run，跳过 sync。确认后在目标仓执行：node scripts/agent-config/sync.mjs");
    process.exit(0);
  }
  if (args.noSync) {
    console.error("land: --no-sync。请在目标仓执行：node scripts/agent-config/sync.mjs");
    process.exit(0);
  }
