#!/usr/bin/env node
/**
 * fill-merge-redis — merge docs/redis/.fill-work fragments into SSOT (0.2.19+).
 * Compat (0.3.7+): prefer `fill-merge.mjs --domain redis`.
 *
 * Usage:
 *   node scripts/fill-merge-redis.mjs --inventory inv.json --work-dir <root>/docs/redis/.fill-work --check
 *   node scripts/fill-merge-redis.mjs --inventory inv.json --work-dir ... --target docs/redis/keys/01-x.md --write
 *   node scripts/fill-merge-redis.mjs ... --write --force-write   # skip acceptance + missing check
 *   node scripts/fill-merge-redis.mjs --help
 *
...
function printHelp() {
  console.log(`Usage:
  node scripts/fill-merge-redis.mjs --inventory inv.json --work-dir <dir> --check
  node scripts/fill-merge-redis.mjs --inventory inv.json --work-dir <dir> --target <ssot.md> --write
  node scripts/fill-merge-redis.mjs ... --write --force-write

Options:
  --inventory   fill-inventory-redis JSON
  --work-dir    directory of shard fragments (*.md)
  --target      SSOT keys truth path (required with --write)
