 ...
 * 0.6.4-dev: CodeBuddy/WorkBuddy flat rules.md + FM; hooks/MCP/permissions docs.
...
assert(/version:\s*"0\.6\.4-dev"/.test(qYaml), "questions.yaml version 0.6.4-dev");
...
assert(/version:\s*"0\.6\.4-dev"/.test(manifest), "manifest 0.6.4-dev");
...
assert(/skill_version:\s*"0\.6\.4-dev"/.test(metaTmpl), "harness-meta 0.6.4-dev");
...
assert(/## 0\.6\.4-dev/.test(changelog), "CHANGELOG 0.6.4-dev");
...
assert(/验收记录（0\.6\.4-dev）/.test(verifyMd) && /当前 \*\*0\.6\.4-dev\*\*/.test(verifyMd), "VERIFY is 0.6.4-dev");
...
assert(/当前版本：0\.6\.4-dev/.test(readme), "README header version 0.6.4-dev");
assert(/当前 \*\*0\.6\.4-dev\*\*/.test(readme), "README footer version 0.6.4-dev");
...
assert(/版本：\*\*0\.6\.4-dev\*\*/.test(handbookMd), "使用手册.md version 0.6.4-dev");
assert(/v0\.6\.4-dev/.test(handbookHtml), "使用手册.html version 0.6.4-dev");
assert(/当前 \*\*0\.6\.4-dev\*\*/.test(quickstartMd), "QUICKSTART version 0.6.4-dev");
...
// --- 0.3.5 gold profile ---
{
  ...
  assert(/version:\s*"0\.6\.4-dev"/.test(fs.readFileSync(path.join(skillRoot, "templates/_meta/manifest.yaml"), "utf8")), "manifest 0.6.4-dev");
}
