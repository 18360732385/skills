 ...
 *
 * HARNESS_SYNC_TMPL_ID: 0.7.14
 * HARNESS_ENG_VERSION: 0.7.14
 *
 * 用法：
 ...
 *   MCP → .codex/config.toml.example（enabled/approval 来自 mcp/policy.json）；hooks → .codex/hooks.json +
 *   codex-adapter；skills → .agents/skills/（不全量 prune 用户自建 skill 目录）。
...
const HARNESS_SYNC_TMPL_ID = "0.7.14";
const HARNESS_ENG_VERSION = "0.7.14";
...
/** Optional L4 rulehook — shape matches upstream rulehook adapters/codex.py install(). */
function isRulehookCommand(command) {
  return /rulehook['"]?\s+hook\b/.test(String(command || ""));
}

function mergeRulehookCodexHooks(doc, opts = {}) {
...
function wantRulehookMerge() {
  return fs.existsSync(path.join(ROOT, ".rulehook", "rulehook.toml"));
}

function finalizeCodexHooksDoc(doc) {
  if (wantRulehookMerge()) mergeRulehookCodexHooks(doc);
  return doc;
}
...
function planCodexHooks(allScripts, scriptsDir) {
  ...
  if (ssotExists("codex", "hooks.json")) {
    try {
      hooksDoc = JSON.parse(readSsot("codex", "hooks.json"));
    } catch {
      hooksDoc = { hooks: {} };
    }
  } else {
  ...
  putJson(".codex/hooks.json", finalizeCodexHooksDoc(hooksDoc));
  for (const s of allScripts) {
  ...
  if (ssotExists("hooks", "codex-hook.cmd")) {
    put(".codex/hooks/codex-hook.cmd", readSsot("hooks", "codex-hook.cmd"));
  } else {
    const cmdPath = path.join(scriptsDir, "codex-hook.cmd");
    if (fs.existsSync(cmdPath)) put(".codex/hooks/codex-hook.cmd", fs.readFileSync(cmdPath));
  }
}
...
function looksSafeReadonlyTool(name) {
...
/** servers[name] > heuristics > defaults；无 policy 时写库关、安全工具开、其余关。 */
function resolveMcpServerPolicy(name, policy) {
...
function loadMcpPolicy() {
  if (!ssotExists("mcp", "policy.json")) return null;
  try {
    return JSON.parse(readSsot("mcp", "policy.json"));
  } catch {
    return null;
  }
}

/** Inline MCP JSON → Codex TOML (no secrets; policy-aware). */
function jsonServersToCodexToml(doc, policy) {
...
function planMcp() {
  ...
  if (ssotExists("mcp", "servers.json")) {
    ...
    if (has("codex")) {
      put(".codex/config.toml.example", jsonServersToCodexToml(servers, policy));
    }
  }
  if (ssotExists("mcp", "servers.example.json")) {
    ...
    if (has("codex") && !plan.has(".codex/config.toml.example")) {
      put(".codex/config.toml.example", jsonServersToCodexToml(example, policy));
    }
  }
}
