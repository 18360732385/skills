...
- **Config / MCP**: `.codex/config.toml.example`（由 `mcp/servers*.json` 脱敏 + **`mcp/policy.json`** 决定 `enabled`/`approval_mode`）；本机 trusted 后 `.codex/config.toml`（**gitignore**）；会话 `/mcp`；**calibrate-live** 可读项目 toml（`env_vars`→本机 env）
- **Skills**: L5 `docs/agent-config/skills/` → `.agents/skills/`（种子：`contract-sync` / `api|db|redis|jobs-doc-sync` / `frontend-web`；不 prune 用户自建）
- **NL 分层**：AGENTS+docs（SSOT）→ Skills（域引导）→ Starlark（命令）→ harness soft hooks → **可选 L4 rulehook**（`Q_RULEHOOK` / 已有 `.rulehook/` 才落盘；sync 合并 `rulehook hook --target codex`；不默认依赖）；详 [CODEX-PARITY.md](../../../host/CODEX-PARITY.md) · [rulehook/README.md](../../agent-config/rulehook/README.md)
