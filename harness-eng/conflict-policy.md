...
| L5 已启用（meta `agent_config: true`） | 写盘入口 **`scripts/land.mjs`**：`.cursor/rules` 改投 `docs/agent-config/rules/`；hooks 脚本改投 `docs/agent-config/hooks/`；`.cursor/hooks.json` / `.claude/settings.json` / CLAUDE.md 由 `sync.mjs` 生成，**render / land 不直渲**（互斥，避免被 sync 当 stale 清理或漂移）；全量镜像宿主 **omit** 冗余 `1x-contract-sync`（Codex 仍写 `.codex/contract-sync.md`） | — |
