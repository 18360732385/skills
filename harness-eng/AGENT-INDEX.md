...
| 会话自证（当前宿主 · **1.2** 人工闸 + playbook） | [modes/session-live.md](modes/session-live.md) · `scripts/session-live.mjs --help`（`--emit-playbook` / `--merge-matrix`） |
| audit 内容扫描（分册空壳 / Never do↔Pn） | `scripts/content-shell-scan.mjs --root <TARGET>` |
