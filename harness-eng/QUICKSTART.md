...
```bash
node -e "require('fs').writeFileSync('params.json', JSON.stringify({ladder:'L4',domains:['api'],expandFromManifest:true,placeholders:{PROJECT_NAME:'x',PROJECT_DESC:'x',CODE_PREFIXES:'src/',SKILL_VERSION:'0.7.6'}}), 'utf8')"
node scripts/harness.mjs --root <TARGET> --params params.json --mode land
```
...
版本见 [CHANGELOG.md](CHANGELOG.md)（当前 **0.7.6**）。
