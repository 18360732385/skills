...
2. `.cursor/mcp.json` 经确认写入；example / 使用说明已与命名约定对齐
...
| 路径 | 条件 |
|---|---|
| **A · MCP** | 应有矩阵中每个 server 已写入 `mcp.json`；且至少一【推荐】主环境（优先 **`test`**，其次 `meta.fill_mcp_profile`，再否则列表首个有凭证环境）在本会话可调用扫库 |
...
写完 `.cursor/mcp.json` 后，若会话工具列表仍无对应 server：
...
```
- [ ] 1 读 Fingerprint：S_DB_ENGINE / S_STACK_REDIS / S_ENV_PROFILES / 应有矩阵
- [ ] 2 确认 L4 example；按矩阵扩展 example 占位（无真密）
- [ ] 3 从本仓对应 profile 的 yml/properties **读取**连接信息
- [ ] 4 WritePlan：mcp.json 将含哪些 {engine}-{profile} — 确认
- [ ] 5 写入；gitignore 建议忽略真密 mcp.json
...
- fill-mcp **经确认后可创建/覆盖**目标仓 `.cursor/mcp.json`；已存在须在 WritePlan 标明 merge 或 backup-create
