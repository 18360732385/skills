#!/usr/bin/env node
/**
 * Lightweight 0.2.28 selfcheck: W5 prune + inherited 0.2.27 gates.
 */
import fs from "fs";
...
assert(/version:\s*"0\.2\.28"/.test(qYaml), "questions.yaml version 0.2.28");
...
assert(/version:\s*"0\.2\.28"/.test(manifest), "manifest 0.2.28");
const metaTmpl = fs.readFileSync(
  path.join(skillRoot, "templates/meta/harness-meta.yaml.tmpl"),
  "utf8"
);
assert(/skill_version:\s*"0\.2\.28"/.test(metaTmpl), "harness-meta 0.2.28");
...
assert(/## 0\.2\.28/.test(changelog), "CHANGELOG 0.2.28");
...
const selfchecksHot = scriptsDir.filter((n) => /^selfcheck-0\.2\.\d+\.mjs$/.test(n));
assert(
  selfchecksHot.length === 1 && selfchecksHot[0] === "selfcheck-0.2.28.mjs",
  "only current selfcheck in scripts/"
);
assert(
  fs.existsSync(path.join(skillRoot, "archive/selfcheck/selfcheck-0.2.27.mjs")),
  "old selfcheck archived"
);
assert(
...
assert(/0\.2\.28/.test(verifyMd), "VERIFY is 0.2.28");
...
assert(/当前版本：0\.2\.28/.test(readme), "README header version 0.2.28");
assert(/当前 \*\*0\.2\.28\*\*/.test(readme), "README footer version 0.2.28");
...
assert(/selfcheck-0\.2\.28/.test(readme), "README pins selfcheck 0.2.28");
...
if (fs.existsSync(fixture)) {
  ...
  assert(["0.2.18","0.2.19","0.2.20","0.2.21","0.2.22","0.2.23","0.2.24","0.2.25","0.2.26","0.2.27","0.2.28"].includes(ui.version), "ui.version compatible");
...
console.log("selfcheck-0.2.28 PASS");
