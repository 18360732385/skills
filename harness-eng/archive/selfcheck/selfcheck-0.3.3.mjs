#!/usr/bin/env node
/**
 * Lightweight 0.3.3 selfcheck: wfa closeout + inherited 0.2.27–0.3.1 gates.
 */
import fs from "fs";
...
assert(/version:\s*"0\.3\.3"/.test(qYaml), "questions.yaml version 0.2.29");
...
assert(/version:\s*"0\.3\.3"/.test(manifest), "manifest 0.2.29");
const metaTmpl = fs.readFileSync(
  path.join(skillRoot, "templates/meta/harness-meta.yaml.tmpl"),
  "utf8"
);
assert(/skill_version:\s*"0\.3\.3"/.test(metaTmpl), "harness-meta 0.2.29");
...
assert(/## 0\.3\.3/.test(changelog), "CHANGELOG 0.2.29");
...
assert(
  selfchecksHot.length === 1 && selfchecksHot[0] === "selfcheck-0.3.3.mjs",
...
assert(/0\.3\.3/.test(verifyMd), "VERIFY is 0.2.29");
...
assert(/当前版本：0\.3\.3/.test(readme), "README header version 0.2.29");
assert(/当前 \*\*0\.3\.3\*\*/.test(readme), "README footer version 0.2.29");
...
assert(/selfcheck-0\.3\.3/.test(readme), "README pins selfcheck 0.2.29");
...
// --- 0.3.3 gold profile ---
{
  ...
  assert(/0\.3\.3/.test(fs.readFileSync(path.join(skillRoot, "templates/_meta/manifest.yaml"), "utf8")), "manifest 0.3.3");
}
...
console.log("selfcheck-0.3.3 PASS");
