#!/usr/bin/env node
/**
 * Lightweight 0.2.22 selfcheck (inherits 0.2.21): MCP matrix + fill MCP gate docs.
 */
import fs from "fs";
...
assert(/version:\s*"0\.2\.22"/.test(qYaml), "questions.yaml version 0.2.22");
assert(
...
assert(/version:\s*"0\.2\.22"/.test(manifest), "manifest 0.2.22");
const metaTmpl = fs.readFileSync(
  path.join(skillRoot, "templates/meta/harness-meta.yaml.tmpl"),
  "utf8"
);
assert(/skill_version:\s*"0\.2\.22"/.test(metaTmpl), "harness-meta 0.2.22");
const changelog = fs.readFileSync(path.join(skillRoot, "CHANGELOG.md"), "utf8");
assert(/## 0\.2\.22/.test(changelog), "CHANGELOG 0.2.22");
...
assert(/当前版本：0\.2\.22/.test(readme), "README version 0.2.22");
assert(!/selfcheck-0\.2\.15/.test(readme), "README does not pin stale selfcheck 0.2.15");
assert(/selfcheck-0\.2\.22/.test(readme), "README pins selfcheck 0.2.22");
...
assert(/填充 MCP 闸/.test(fs.readFileSync(path.join(skillRoot, "pipeline.md"), "utf8")), "pipeline has fill MCP gate");
...
if (fs.existsSync(fixture)) {
  ...
  assert(["0.2.18","0.2.19","0.2.20","0.2.21","0.2.22","0.2.23","0.2.24"].includes(ui.version), "ui.version compatible");
...
console.log("selfcheck-0.2.22 PASS");
