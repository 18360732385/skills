#!/usr/bin/env node
/**
 * Lightweight 0.3.5 selfcheck: wfa closeout + inherited 0.2.27–0.3.1 gates.
 */
import fs from "fs";
...
assert(/version:\s*"0\.3\.5"/.test(qYaml), "questions.yaml version 0.3.5");
...
assert(/version:\s*"0\.3\.5"/.test(manifest), "manifest 0.3.5");
const metaTmpl = fs.readFileSync(
  path.join(skillRoot, "templates/meta/harness-meta.yaml.tmpl"),
  "utf8"
);
assert(/skill_version:\s*"0\.3\.5"/.test(metaTmpl), "harness-meta 0.3.5");
...
assert(/## 0\.3\.5/.test(changelog), "CHANGELOG 0.3.5");
...
assert(
  selfchecksHot.length === 1 && selfchecksHot[0] === "selfcheck-0.3.5.mjs",
...
assert(/0\.3\.5/.test(verifyMd), "VERIFY is 0.3.5");
...
assert(/当前版本：0\.3\.5/.test(readme), "README header version 0.3.5");
assert(/当前 \*\*0\.3\.5\*\*/.test(readme), "README footer version 0.3.5");
...
assert(/selfcheck-0\.3\.5/.test(readme), "README pins selfcheck 0.3.5");
...
// --- 0.3.5 gold profile ---
{
  ...
  assert(/0\.3\.5/.test(fs.readFileSync(path.join(skillRoot, "templates/_meta/manifest.yaml"), "utf8")), "manifest 0.3.5");
}
...
// --- 0.3.4/0.3.5 jobs domain + domains.yaml + packs + inventory ---
{
  ...
  assert(
    fs.existsSync(path.join(skillRoot, "archive/selfcheck/selfcheck-0.3.4.mjs")),
    "0.3.4 selfcheck archived"
  );
...
console.log("selfcheck-0.3.5 PASS");
