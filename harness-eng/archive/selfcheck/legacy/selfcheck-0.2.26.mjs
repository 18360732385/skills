#!/usr/bin/env node
/**
 * Lightweight 0.2.26 selfcheck: ready_coverage 0.8 · MCP profile test · api-empty-examples.
 */
import fs from "fs";
...
assert(/version:\s*"0\.2\.26"/.test(qYaml), "questions.yaml version 0.2.26");
...
assert(/version:\s*"0\.2\.26"/.test(manifest), "manifest 0.2.26");
const metaTmpl = fs.readFileSync(
  path.join(skillRoot, "templates/meta/harness-meta.yaml.tmpl"),
  "utf8"
);
assert(/skill_version:\s*"0\.2\.26"/.test(metaTmpl), "harness-meta 0.2.26");
...
assert(/## 0\.2\.26/.test(changelog), "CHANGELOG 0.2.26");
...
assert(/当前版本：0\.2\.26/.test(readme), "README version 0.2.26");
assert(!/selfcheck-0\.2\.15/.test(readme), "README does not pin stale selfcheck 0.2.15");
assert(/selfcheck-0\.2\.26/.test(readme), "README pins selfcheck 0.2.26");
...
if (fs.existsSync(fixture)) {
  ...
  assert(["0.2.18","0.2.19","0.2.20","0.2.21","0.2.22","0.2.23","0.2.24","0.2.25","0.2.26"].includes(ui.version), "ui.version compatible");
...
console.log("selfcheck-0.2.26 PASS");
