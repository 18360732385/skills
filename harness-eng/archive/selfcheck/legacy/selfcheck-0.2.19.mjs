#!/usr/bin/env node
/**
 * Lightweight 0.2.19 selfcheck: multi-module merge, domain merge scripts, inventory fix,
...
assert(/version:\s*"0\.2\.19"/.test(qYaml), "questions.yaml version 0.2.19");
assert(
...
assert(/version:\s*"0\.2\.19"/.test(manifest), "manifest 0.2.19");
const metaTmpl = fs.readFileSync(
  path.join(skillRoot, "templates/meta/harness-meta.yaml.tmpl"),
  "utf8"
);
assert(/skill_version:\s*"0\.2\.19"/.test(metaTmpl), "harness-meta 0.2.19");
const changelog = fs.readFileSync(path.join(skillRoot, "CHANGELOG.md"), "utf8");
assert(/## 0\.2\.19/.test(changelog), "CHANGELOG 0.2.19");

const skill = fs.readFileSync(path.join(skillRoot, "SKILL.md"), "utf8");
assert(/0\.2\.19/.test(skill), "SKILL 0.2.19");
...
if (fs.existsSync(fixture)) {
  ...
  assert(ui.version === "0.2.18" || ui.version === "0.2.19", "ui.version compatible");
}
...
console.log("selfcheck-0.2.19 PASS");
