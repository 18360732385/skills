#!/usr/bin/env node
/**
 * Lightweight 0.3.9 selfcheck: source-repo discipline backfill + inherited 0.2.27–0.3.8 gates.
 */
import fs from "fs";
...
assert(/version:\s*"0\.3\.9"/.test(qYaml), "questions.yaml version 0.3.9");
...
assert(/version:\s*"0\.3\.9"/.test(manifest), "manifest 0.3.9");
const metaTmpl = fs.readFileSync(
  path.join(skillRoot, "templates/meta/harness-meta.yaml.tmpl"),
  "utf8"
);
assert(/skill_version:\s*"0\.3\.9"/.test(metaTmpl), "harness-meta 0.3.9");
...
assert(/## 0\.3\.9/.test(changelog), "CHANGELOG 0.3.9");
...
assert(
  selfchecksHot.length === 1 && selfchecksHot[0] === "selfcheck-0.3.9.mjs",
...
assert(/0\.3\.9/.test(verifyMd), "VERIFY is 0.3.9");
...
assert(/当前版本：0\.3\.9/.test(readme), "README header version 0.3.9");
assert(/当前 \*\*0\.3\.9\*\*/.test(readme), "README footer version 0.3.9");
...
assert(/selfcheck-0\.3\.9/.test(readme), "README pins selfcheck 0.3.9");
...
// --- 0.3.5 gold profile ---
{
  ...
  assert(/0\.3\.9/.test(fs.readFileSync(path.join(skillRoot, "templates/_meta/manifest.yaml"), "utf8")), "manifest 0.3.9");
}
...
console.log("selfcheck-0.3.9 PASS");
