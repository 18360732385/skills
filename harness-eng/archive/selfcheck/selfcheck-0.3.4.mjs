#!/usr/bin/env node
/**
 * Lightweight 0.3.4 selfcheck: wfa closeout + inherited 0.2.27–0.3.1 gates.
 */
import fs from "fs";
...
import { parseDomainsYaml, resolveScoreDomains } from "./lib/domains.mjs";
...
assert(/version:\s*"0\.3\.4"/.test(qYaml), "questions.yaml version 0.3.4");
...
assert(/version:\s*"0\.3\.4"/.test(manifest), "manifest 0.3.4");
const metaTmpl = fs.readFileSync(
  path.join(skillRoot, "templates/meta/harness-meta.yaml.tmpl"),
  "utf8"
);
assert(/skill_version:\s*"0\.3\.4"/.test(metaTmpl), "harness-meta 0.3.4");
...
assert(/## 0\.3\.4/.test(changelog), "CHANGELOG 0.3.4");
...
assert(
  selfchecksHot.length === 1 && selfchecksHot[0] === "selfcheck-0.3.4.mjs",
...
assert(/0\.3\.4/.test(verifyMd), "VERIFY is 0.3.4");
...
assert(/当前版本：0\.3\.4/.test(readme), "README header version 0.3.4");
assert(/当前 \*\*0\.3\.4\*\*/.test(readme), "README footer version 0.3.4");
...
assert(/selfcheck-0\.3\.4/.test(readme), "README pins selfcheck 0.3.4");
...
// --- 0.3.4 gold profile ---
{
  ...
  assert(/0\.3\.4/.test(fs.readFileSync(path.join(skillRoot, "templates/_meta/manifest.yaml"), "utf8")), "manifest 0.3.4");
}

// --- 0.3.4 jobs domain + domains.yaml ---
{
  assert(
  ...
  assert(/id:\s*jobs-index/.test(man) || /domain:\s*jobs/.test(man), "manifest jobs files");
  ...
  assert(
    fs.existsSync(path.join(skillRoot, "archive/selfcheck/selfcheck-0.3.3.mjs")),
    "0.3.3 selfcheck archived"
  );
...
console.log("selfcheck-0.3.4 PASS");
