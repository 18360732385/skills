...
## 0.6.3-dev — 2026-09-14（sync.mjs freshness gate + 安装 URL V0.6.X）

Audit P0-1。技能升级后消费仓实例化 `scripts/agent-config/sync.mjs` 不再静默沿用旧 tmpl（Trae FM strip 等）。**不**改 Trae 矩阵、**不**重开 Codex、**不**并 `main`。
...
- **安装 URL**：0.6 列车文档（README / QUICKSTART / 手册）指向 `tree/V0.6.X/harness-eng`（开发跟 `V0.6.X`；已发布快照可见 `main`）
