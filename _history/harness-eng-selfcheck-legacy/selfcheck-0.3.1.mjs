#!/usr/bin/env node
/**
 * Lightweight 0.2.29 selfcheck: Phase C isomorphic + morph/gate + inherited 0.2.27 gates.
 */
import fs from "fs";
...
assert(/version:\s*"0\.3\.1"/.test(qYaml), "questions.yaml version 0.2.29");
...
assert(/version:\s*"0\.3\.1"/.test(manifest), "manifest 0.2.29");
const metaTmpl = fs.readFileSync(
  path.join(skillRoot, "templates/meta/harness-meta.yaml.tmpl"),
  "utf8"
);
assert(/skill_version:\s*"0\.3\.1"/.test(metaTmpl), "harness-meta 0.2.29");
...
assert(/## 0\.3\.1/.test(changelog), "CHANGELOG 0.2.29");
...
assert(
  selfchecksHot.length === 1 && selfchecksHot[0] === "selfcheck-0.3.1.mjs",
  "only current selfcheck in scripts/"
);
assert(
  fs.existsSync(path.join(skillRoot, "archive/selfcheck/selfcheck-0.3.0.mjs")),
  "0.3.0 selfcheck archived"
);
assert(
...
assert(/0\.3\.1/.test(verifyMd), "VERIFY is 0.2.29");
...
assert(/当前版本：0\.3\.1/.test(readme), "README header version 0.2.29");
assert(/当前 \*\*0\.3\.1\*\*/.test(readme), "README footer version 0.2.29");
...
assert(/selfcheck-0\.3\.1/.test(readme), "README pins selfcheck 0.2.29");
...
// --- 0.3.1 Phase C ---
{
  ...
  assert(/0\.3\.1 计分同构/.test(fs.readFileSync(path.join(skillRoot, "templates/docs/redis/templates/redis-key-template.md"), "utf8")), "redis tmpl note");
  assert(/0\.3\.1 计分同构/.test(fs.readFileSync(path.join(skillRoot, "templates/docs/db/templates/db-table-template.md"), "utf8")), "db tmpl note");
}
...
console.log("selfcheck-0.3.1 PASS");
