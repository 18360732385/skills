#!/usr/bin/env node
/**
 * Lightweight 0.5.1 selfcheck: 配置 SSOT 管线 L5 / hooks 家族 / pitfalls 工程化
...
assert(/version:\s*"0\.5\.1"/.test(qYaml), "questions.yaml version 0.5.1");
...
assert(/version:\s*"0\.5\.1"/.test(manifest), "manifest 0.5.1");
const metaTmpl = fs.readFileSync(
  path.join(skillRoot, "templates/meta/harness-meta.yaml.tmpl"),
  "utf8"
);
assert(/skill_version:\s*"0\.5\.1"/.test(metaTmpl), "harness-meta 0.5.1");
...
assert(/## 0\.5\.1/.test(changelog), "CHANGELOG 0.5.1");
...
assert(
  selfchecksHot.length === 1 && selfchecksHot[0] === "selfcheck-0.5.1.mjs",
...
assert(/0\.5\.1/.test(verifyMd), "VERIFY is 0.5.1");
...
assert(/当前版本：0\.5\.1/.test(readme), "README header version 0.5.1");
assert(/当前 \*\*0\.5\.1\*\*/.test(readme), "README footer version 0.5.1");
...
assert(/selfcheck-0\.5\.1/.test(readme), "README pins selfcheck 0.5.1");
...
// --- 0.3.5 gold profile ---
{
  ...
  assert(/0\.5\.1/.test(fs.readFileSync(path.join(skillRoot, "templates/_meta/manifest.yaml"), "utf8")), "manifest 0.5.1");
}
...
// --- 0.5.1 static: L5 配置 SSOT 管线 / hooks 家族 / pitfalls 工程化 ---
{
  // L5 模板包存在性 + 去域化
  for (const rel of [
...
// --- 0.5.1 runtime: L4 hooks 家族直渲 + L5 render→sync --check + lint-pitfalls 好坏例 ---
{
  ...
  try {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ladder: "L4",
        domains: ["func", "api", "db"],
        ai_tools: ["cursor", "claude", "qoder", "trae"],
        agents_variant: "solo",
        hooks_family: family,
        expandFromManifest: true,
    ...
    assert(r.status === 0, "0.5.1 L4 render exits 0");
  ...
  try {
    ...
    assert(r.status === 0, "0.5.1 L5 render exits 0");
    assert(
...
console.log("selfcheck-0.5.1 PASS");
