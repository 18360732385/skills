#!/usr/bin/env node
/**
 * Lightweight 0.2.20 selfcheck (inherits 0.2.19 + doc asserts): multi-module merge, domain merge scripts, inventory fix,
 * score output standardization, variant naming, shard progress.
 */
import fs from "fs";
...
assert(/version:\s*"0\.2\.20"/.test(qYaml), "questions.yaml version 0.2.20");
assert(
...
assert(/version:\s*"0\.2\.20"/.test(manifest), "manifest 0.2.20");
const metaTmpl = fs.readFileSync(
  path.join(skillRoot, "templates/meta/harness-meta.yaml.tmpl"),
  "utf8"
);
assert(/skill_version:\s*"0\.2\.20"/.test(metaTmpl), "harness-meta 0.2.20");
const changelog = fs.readFileSync(path.join(skillRoot, "CHANGELOG.md"), "utf8");
assert(/## 0\.2\.20/.test(changelog), "CHANGELOG 0.2.20");
...
assert(/当前版本：0\.2\.20/.test(readme), "README version 0.2.20");
...
if (fs.existsSync(fixture)) {
  ...
  assert(["0.2.18","0.2.19","0.2.20"].includes(ui.version), "ui.version compatible");
}
...
console.log("selfcheck-0.2.20 PASS");
