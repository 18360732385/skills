...
当前 **0.3.20-dev**（叠在 0.3.19-dev：模式 md 迁入 `modes/`；薄入口与 P0 门禁包继承）。权威号见 [`_meta/manifest.yaml`](_meta/manifest.yaml)。变更见 [CHANGELOG.md](CHANGELOG.md)。

本版**不**改发版仪式语义与既有脚本行为契约（相对 0.3.19-dev）；仅迁模式 md 路径与全量改链。
...
| manifest / CHANGELOG / README / SKILL / AGENT-INDEX / QUICKSTART / VERIFY 钉 **0.3.20-dev** | 有 |
| CHANGELOG 含 `## 0.3.20-dev` 且保留 `## 0.3.19-dev` / `## 0.3.18-dev` | 有 |
